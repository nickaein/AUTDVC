...
VectorEx<double> OpticalFlow::LapPara;
...
void OpticalFlow::estLaplacianNoise(const DImage& Im1,const DImage& Im2,VectorEx<double>& para)
{
	...
	if(para.dim()!=nChannels)
		...
		para.reset();
	double temp;
	VectorEx<double> total(nChannels);
	for(int k = 0;k<nChannels;k++)
		total[k] = 0;
